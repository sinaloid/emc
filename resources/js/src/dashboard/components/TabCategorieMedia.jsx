...
const TabCategorieMedia = ({ setBtnName = () => {}, setRestForm}) => {
    const close = useRef()
    ...
    useEffect(() =>{
        setBtnName("Ajouter une catégorie média");
        setRestForm(formik)
        get()
    },[])
    const btnEditProps = {
    ...
    return (
        <>
                    ...
                    <tbody>
                        {datas.map((data, idx) => {
                            return (
                                <tr key={idx}>
                                    <td>{idx + 1}</td>
                                    <td><img width="30px" src={data.image ? URL+""+ data.image : ""} alt="" /></td>
                                    <td>{data.name}</td>
                                    <td ><p className="text-container">{data.description}</p></td>
                                    <td className="text-center">
                                        <ActionButton btnEditProps={btnEditProps} data={data} editData={editData} destroy={destroy} view={view} />
                                    </td>
                                </tr>
                            );
                        })}
                    ...
                    <div className="modal-content">
                        <div className="modal-header border-0">
                            <h4 className="modal-title text-meduim text-bold">
                                ...
                                {
                                    formik.values["slug"] !=="" ? "Modification de la catégorie de média" :"Ajout d’une catégorie de média"
                                }
                            </h4>
                            <button
                                type="button"
            ...
            <button ref={viewRef} data-bs-toggle="modal" data-bs-target="#view" hidden></button>
                        ...
                        <div className="modal-body">
                            <div className="row">
                                <div className="col-12 col-md-4">
                                    <img width="100%" src={viewData.image ? URL+""+viewData.image : ""} alt="" />
                                </div>
