...
const TabCategorieFiltre = ({ setBtnName = () => {}, setRestForm }) => {
    const close = useRef();
    ...
    useEffect(() => {
        setBtnName("Ajouter une catégorie filtre");
        setRestForm(formik);
        get();
    }, []);
    const btnEditProps = {
        "data-bs-target": "#categorieFiltre",
    };
    ...
    const post = async (values) => {
        const response = await toast.promise(
            request.post(endPoint.categorieFiltres, values),
            {
    ...
    const view = async (values) => {
        const response = await toast.promise(
            request.get(endPoint.categorieFiltres + "/" + values.slug),
            {
                pending: "Veuillez patienté...",
                success: {
                    render({ data }) {
    ...
    const update = async (values) => {
        const response = await toast.promise(
            request.post(endPoint.categorieFiltres + "/" + values.slug, values),
            {
                pending: "Veuillez patienté...",
                success: {
                    render({ data }) {
    ...
    const destroy = async (values) => {
        const response = await toast.promise(
            request.delete(endPoint.categorieFiltres + "/" + viewData.slug),
            {
                pending: "Veuillez patienté...",
                success: {
                    render({ data }) {
    ...
    return (
        <>
            ...
            <div className="modal fade" id="categorieFiltre">
